...
interface ConstructionArchiveProps {
...
export default function ConstructionArchive({ currentUser, canEdit = true, canDelete = true, preselectedProjectId, initialDetailTab, onEditQuote }: ConstructionArchiveProps) {
  ...
  // Tải động html2canvas-pro/jsPDF rồi tự ghép ảnh vào PDF (tự chia trang nếu
  // nội dung dài hơn 1 trang A4) — theo đúng cách export PDF đã ổn định của
  // Đơn Mua Hàng (MaterialCoordination.tsx), KHÔNG dùng html2pdf.js (thư viện
  // đó gán nhầm property `container.height` thay vì `.style.height` khiến
  // ảnh chụp ra trắng hoàn toàn — đã xác minh ở đó).
  // Lưu ý: dùng "html2canvas-pro" (fork còn bảo trì) thay vì "html2canvas"
  // gốc — bản gốc (v1.4.1, không còn cập nhật) ném lỗi "unsupported color
  // function oklch" ngay khi gặp bảng màu mặc định của Tailwind v4 dùng
  // trong giao diện màn hình (Đơn Mua Hàng không dính lỗi này vì dựng HTML
  // tĩnh bằng màu hex, không qua Tailwind).
  ...
  useEffect(() => {
    if (selectedQuote) {
      const candidateName = selectedQuote.projectName ||
                            ...
                            `Dự án ${selectedQuote.customerName || 'vãng lai'} - Lập Xây dựng`;
  ...
  const handleQuickCreateProject = async () => {
    ...
    try {
      ...
      addToast({ title: '✅ Thành công', message: `Khởi tạo dự án "${quickProjName}" thành công!`, type: 'success' });

      const updatedQuote = { ...selectedQuote, projectId: generatedProjId };
      await dbService.archivedQuotes.save({ ...updatedQuote, sector: 'construction' });
      window.dispatchEvent(new CustomEvent('hl-archived-quotes-updated'));
      setSelectedQuote(updatedQuote);

      const data = await dbService.archivedQuotes.list('construction');
  ...
  const fetchArchives = async () => {
    ...
    try {
      ...
      const [data, projs, custs] = await Promise.all([
        dbService.archivedQuotes.list('construction'),
      ...
      console.error("Lỗi khi tải hồ sơ báo giá xây dựng:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchArchives();
    const handleConstructionUpdated = () => fetchArchives();
    ...
    <div className="bg-slate-900 text-slate-100 rounded-2xl border border-slate-800 p-6 space-y-6 text-left" id="construction_archive_workspace">
      ...
      <div className="flex gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800/80">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-slate-900 border border-slate-800 pl-10 pr-4 py-2.5 rounded-lg text-xs outline-none focus:border-indigo-500 text-slate-100 placeholder-slate-500 font-medium"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-12 text-center text-xs text-slate-500 animate-pulse font-bold uppercase tracking-wider">
          Đang tải dữ liệu hồ sơ xây dựng...
        </div>
        ...
        <div className="py-12 text-center border border-dashed border-slate-800 rounded-xl bg-slate-900/10 space-y-2">
          ...
          <p className="text-[10px] text-slate-500 max-w-sm mx-auto">
            Vui lòng nhấn "Lưu & In" tại bảng lập dự toán xây dựng để khởi tạo bản ghi mới tự động.
          </p>
        </div>
      ) : (
                  ...
                  return (
                    <React.Fragment key={proj?.id || `unlinked-${groupIdx}`}>
                      ...
                      {quotes.map((item) => {
                        ...
                        const scaleStr = itemAny.chieuDai && itemAny.chieuRong
                        ...
                        const docs = [
                          ...
                          },
                          {
                            ...
                            // điều kiện thêm theo contractHtml có tồn tại hay không, vì hồ sơ
                            // có thể đã duyệt (contractApproved=true) mà contractHtml vẫn rỗng
                            // (bản in được sinh động từ template lúc mở xem) — trước đây điều
                            // kiện !item.contractHtml khiến các hồ sơ dạng này hiện sai thành
                            // "Chờ Duyệt" dù đã duyệt thật.
                            statusLabel: itemAny.contractApproved ? 'Đã Duyệt' : 'Chờ Duyệt',
                            statusColor: itemAny.contractApproved
                              ? 'bg-white text-emerald-600 border-emerald-500/30 shadow-sm'
                              : 'bg-white text-amber-600 border-amber-500/30 shadow-sm'
                          },
                          {
                            ...
                            statusLabel: itemAny.acceptanceApproved ? 'Đã Duyệt' : 'Chờ Duyệt',
                            statusColor: itemAny.acceptanceApproved
                              ? 'bg-white text-emerald-600 border-emerald-500/30 shadow-sm'
                              : 'bg-white text-amber-600 border-amber-500/30 shadow-sm'
                          },
                          {
                        ...
                        return docs.map((doc) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 font-bold font-mono text-indigo-400 uppercase">
                                {doc.code}
                              </td>
                              ...
                              <td className="px-4 py-3 font-semibold text-[11px] text-slate-300">
                                {itemAny.selectedHouseType || 'Nhà dân dụng'}
                              </td>
                              <td className="px-4 py-3 text-[11px] text-slate-400 font-mono">
                                {scaleStr}
                              </td>
                              <td className="px-4 py-3 text-right font-mono font-semibold text-emerald-400">
                                {itemAny.donGiaKhaiToan ? `${itemAny.donGiaKhaiToan.toLocaleString('vi-VN')} đ/m²` : 'Bóc tách chi tiết'}
                              </td>
      ...
      {/* Detail & Print Popup — dùng React Portal render thẳng vào document.body, tách
          hoàn toàn khỏi cây component của ứng dụng (sidebar, layout cha...). Đây là cách
          làm đúng chuẩn để in: nếu vẫn để modal lồng sâu trong cây component như cũ, khi
          nội dung dài nhiều trang, các thẻ cha (fixed, flex, overflow...) sẽ khiến Chrome
          tính toán sai vị trí và in đè chữ lên nhau ở các trang sau. */}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-extrabold text-sm text-slate-900 uppercase tracking-wider">
                    Hồ sơ lưu trữ xây dựng
                  </h4>
                  <p className="text-[10px] text-slate-500 font-medium">Báo giá chi tiết & Dự toán thầu xây thô</p>
                </div>
              </div>
              <button
            ...
            <div className="bg-slate-50 border-t border-slate-200 px-6 py-4 flex justify-between items-center gap-4 print-hide">
              ...
              <div className="flex gap-2.5">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs rounded-xl cursor-pointer flex items-center gap-1.5 transition-all hover:scale-[1.01]"
                >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full text-left space-y-4 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Tên Dự Án Xây Dựng</label>
                <input
                  type="text"
                  value={quickProjName}
