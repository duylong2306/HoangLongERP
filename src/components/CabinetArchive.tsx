...
export default function CabinetArchive({ currentUser, canEdit = true, canDelete = true }: CabinetArchiveProps) {
  ...
  const handleQuickCreateProject = async () => {
    ...
    try {
      ...
      await dbService.archivedQuotes.save({ ...updatedQuote, sector: 'furniture' });
  ...
  useEffect(() => {
    fetchArchives();
    const handleCabinetUpdated = () => fetchArchives();
    ...
    <div className="bg-slate-900 text-slate-100 rounded-2xl border border-slate-800 p-6 space-y-6 text-left" id="cabinet_archive_workspace">
      ...
      <div className="flex gap-3 bg-slate-950 p-4 rounded-xl border border-slate-800/80">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-slate-900 border border-slate-800 pl-10 pr-4 py-2.5 rounded-lg text-xs outline-none focus:border-amber-500 text-slate-100 placeholder-slate-500 font-medium"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="py-12 text-center text-xs text-slate-500 animate-pulse font-bold uppercase tracking-wider">
          Đang tải dữ liệu hồ sơ nội thất gỗ...
        </div>
        ...
        <div className="py-12 text-center border border-dashed border-slate-800 rounded-xl bg-slate-900/10 space-y-2">
          ...
          <p className="text-[10px] text-slate-500 max-w-sm mx-auto">
            Vui lòng nhấn "Lưu & In" tại bảng lập dự toán nội thất để khởi tạo bản ghi mới tự động.
          </p>
        </div>
      ) : (
                  ...
                  return (
                    <React.Fragment key={proj?.id || `unlinked-${groupIdx}`}>
                      ...
                      <tr className="bg-slate-900/60 border-y border-slate-800/80">
                        <td colSpan={9} className="px-4 py-2 text-indigo-400 text-[11px] uppercase tracking-wider font-extrabold">
                      ...
                      {quotes.map((item) => {
                        // Extract materials/finishes from quote items
                        const materialsList = Array.isArray(item.items)
                        ...
                        return docs.map((doc) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 py-3 font-bold font-mono text-amber-500 uppercase">
                                {doc.code}
                              </td>
                              ...
                              <td className="px-4 py-3 max-w-[220px] truncate text-[11px] text-slate-400" title={materialsList}>
                                {materialsList}
                              </td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full text-left space-y-4 shadow-2xl">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Tên Dự Án Gỗ Nội Thất</label>
                <input
                  type="text"
                  value={quickProjName}
